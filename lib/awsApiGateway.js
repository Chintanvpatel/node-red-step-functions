    ...
    s3.upload(params, function (err, res) {
      if (err) {
        ...
        // Trigger Lambda function for insert endpoint into dynamodb
            ...
            msg.Bucket = s3BucketName;
            msg.Key = keyVal;
            if(type === 'service') {
              msg.serviceName = identity.split('-').slice(-1)[0];
            }else{
              msg.serviceName = appId;  
            }
            console.log('msg-----',msg)
            ...
            var env = appname.split('-');
            console.log('env[0]----',env[0]);
            if(env[0] === 'stg'){
              var func = 'stg-wdev-flow-dynamodb-service';
            }
            else {
              var func = 'prod-wdev-flow-dynamodb-service';
            }
            var params = {
              FunctionName: func, 
              Payload: event, 
