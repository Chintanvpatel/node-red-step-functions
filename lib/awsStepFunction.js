...
var awsStepFunction = {
  ...
  save: function (definition, brandId, appId, identity, type, env, environment, log_prefix) {
    return when.promise(function (resolve, reject) {
      var defOrig = Object.assign({}, definition)
      delete definition.node
      var stepfunctions = new AWS.StepFunctions()
      var params = {
        stateMachineArn: 'arn:aws:states:us-east-1:133013689155:stateMachine:' + env + '-' + defOrig.node.id.replace('.', '-') + '-sfunction'
      }
        ...
        if (err) {
          if (err.code === 'StateMachineDoesNotExist') {
            var params = {
              ...
              name: env + '-' + defOrig.node.id.replace('.', '-') + '-sfunction',
            ...
            if(type === 'service') {
              ...
              params.tags.push({
                key: 'Service',
                value: identity.split('-').slice(-1).pop()
              })
              params.tags.push({
                key: 'Environment',
                value: environment
  ...
  return when.promise(function (resolve, reject) {
    ...
    addInitialNode(node.wires[0][0], states).then(states => {
      processNode(node.wires[0][0], allNodes, states,'',lambdaMapping).then(states => {
        states.node = node
        let lastNode = Object.keys(states.States).slice(-1)[0]
        let DefaultCatch = [{ErrorEquals: ["States.ALL"], ResultPath: "$.output", Next: lastNode}]
        Object.keys(states.States).forEach(function(state) {
          if(typeof states.States[state].End === 'undefined') {
            states.States[state]['Catch'] = DefaultCatch
          }
        });
        resolve(states)
      })
  ...
  return when.promise(function (resolve, reject) {
    ...
    processTaskNodes(nodeDetail, previousNode, states,lambdaMapping).then((states) => {
      if (nodeDetail[0].wires.length === 0) {
        states['States'][nodeDetail[0].id.replace('.', '_')]['End'] = true
        resolve(states)
...
var processTaskNodes = function (nodeDetail, previousNode, states,lambdaMapping) {
  return when.promise(function (resolve, reject) {
    if (nodeDetail[0] && nodeDetail[0].type !== 'switch') {
      ...
      states['States'][nodeDetail[0].id.replace('.', '_')]['TimeoutSeconds'] = 1200
      ...
      states['States'][nodeDetail[0].id.replace('.', '_')]['Parameters']['ExecutionDetail.$'] = '$$.Execution'
      if (previousNode) {
